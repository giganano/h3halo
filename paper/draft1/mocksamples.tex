...
be re-derived using our likelihood function as a check that it produces
...
Fig.~\ref{fig:fiducial_mock_corner} shows the posterior distributions derived
from this procedure with the marginalized likelihood distributions along the
diagonal and 2-dimensional cross sections below the diagonal comparing the
best-fit values with their known values.
...
This is the region of chemical space where most of the data are generally
found, so for a given choice of~$\eta$, the~\textit{total} Fe yield is
well constrained observationally.
With only the total precisely determined,~\yfecc~and~\yfeia~are inversely
related, producing the degeneracy seen in Fig.~\ref{fig:fiducial_mock_corner}.
...
This relationship gives rise to the inverse relation between~$\yfeia$ and
$\eta$, whereas the yield-outflow degeneracy produces a direct relationship
between these parameters (see Appendix~\ref{sec:degeneracy}).
