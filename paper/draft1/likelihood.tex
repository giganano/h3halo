...
\begin{itemize}
	...
	\item \script{M}~is the evolutionary track in chemical and age space.
...
The integral in equation~\refp{eq:lnL_with_integral} above indicates that the
...
given the model-predicted evolutionary track~\script{M}~by integrating the
likelihood along its length and incorporating the measurement uncertainties
(shown by the dotted red contours).
In practice,~\script{M}~is computed in a piece-wise linear form by some
