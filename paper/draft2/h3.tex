...
{\color{red}
For this reason, Mg is by far the strongest tracer of the alpha element
abundance, and in comparison to other surveys (e.g., APOGEE), the H3
[$\alpha$/Fe] ratios show the strongest correlation with [Mg/Fe].
}
...
% }
% , whereas in previous sections an alpha element
...
discussion in~\S~\ref{sec:mocks:fiducial}).
...
Fig.~\ref{fig:gse_wukong_eta} shows the inferred mass-loading factors in
...
\par
In Fig.~\ref{fig:comparison}, we compare our best-fit models for GSE and
Wukong/LMS-1.
...
discussion in~\S~\ref{sec:mocks:fiducial}) is accurate for Wukong/LMS-1, then
...
%
% Each point represents the IMF-averaged yield ratio as predicted from tables
% published by~\citet[][black]{Limongi2018},~\citet[][blue]{Sukhbold2016},
% and~\citet[][red]{Nomoto2013}.
% \citet{Limongi2018} additionally report yields for progenitors with initial
% rotation velocities of~$v_\text{rot} = 150$ and 300 km/s.
% The [Mg/Fe] plateau height inferred from our GSE and Wukong/LMS-1 samples are
% noted with grey dotted lines (for the bottom panel, we have simply asserted
% [O/Mg] = 0; see discussion at the end of~\S~\ref{sec:onezone}).
}
...
% Nonetheless, the silver lining is that our GCE fitting method can effectively
% infer yield ratios and marginalize over uncertainties therein.
...
% To this end, we use~\vice's~\texttt{vice.yields.ccsne.fractional} routine,
% which computes IMF-averaged yields for individual elements given built-in
% yield tables taken from the literature (see, e.g.,~\citealt{Griffith2022} or
% the~\vice~science documentation for further details).
% Since the ``knee'' in the best-fit evolutionary track occurs between [Fe/H] =
% $-2$ and~$-3$ in both GSE and Wukong/LMS-1, we focus on this metallicity range.
% Alongside the values inferred from both systems, Fig.~\ref{fig:yield-comparison}
% shows the predicted [Mg/Fe] and [O/Fe] ratios of massive star ejecta predicted
% from the~\citet{Limongi2018} tables at initial rotational velocities
% of~$v_\text{rot} = 0$, $150$, and $300$ km/s.
% We clarify that we have computed the corresponding O and Mg yields for the
% purpose of this discussion rather than assuming metallicity-independent
% values as in previous sections.
% \par
% The [Mg/Fe] plateau is underpredicted at all~$v_\text{rot}$
% where~\citet{Limongi2018} report yields.
% The [O/Fe] plateau, however, is readily explained by the non-rotating models
% but overpredicted by the rotating models.
% This discrepancy is a known problem with massive star models in which Mg is
% underproduced relative to O~\citep[see discussion in][]{Griffith2022}.
% For the non-rotating models, the problem is purely one of Mg underproduction,
% whereas O overproduction is an additional concern for the rotating models.
% \par
% Despite assuming specific O and Mg yields in our GCE models, reproducing our
% chosen normalization with a combination of massive star models and an IMF is
% much less enlightening than the inferred yield ratios.
% Due to the yield-outflow degeneracy (see Appendix~\ref{sec:degeneracy}), our
% GCE models do not constrain the normalization.
% Furthermore, the absolute scale of yields in GCE models need not match that
% predicted by stellar models, because a portion of SN ejecta may be lost
% directly to a hot outflow (e.g.,~\citealt*{Chisholm2018};~\citealt{Cameron2021}).
% The normalization of elemental yields is sensitive to many uncertain processes
% in stellar astrophysics, such as black hole
% formation~\citep[e.g.,][]{Pejcha2015, Ertl2016},
% convection~\citep[e.g.,][]{Chieffi2001}, and mass
% loss~\citep[e.g.,][]{Sukhbold2016}.
}
