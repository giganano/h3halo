...
\begin{itemize}
	...
	For a given data sample~$d$, the goal is to find the model~$m$ which
	maximizes the~\textit{posterior probability}~$L(m | d)$.
	In practice, however, best-fit values are obtained by maximizing the
	\textit{likelihood function}~$L(d | m)$ describing the probability of 
	observing the data~$d$ given the model~$m$.
	Bayes' Theorem relates the two according to:
	\begin{equation}
	\label{eq:bayes_theorem}
	L(m | d) = \frac{L(d | m) L(m)}{L(d)}
	\end{equation}
	where~$L(m)$ is the likelihood of the model (known as the~\textit{prior})
	and~$L(d)$ is the likelihood of the data (known as the~\textit{evidence}).
	...
	\item The likelihood of observing some data~$d$ given some model~$m$ can
	be expressed as the product of the likelihoods of observing each individual
	point~$d_i$:
	...
	\item Computing the likelihood of observing the data point~$d_i$ given the
	model is non-trivial, because there is no definitive way to associate the
	point~$d_i$ with the ``correct'' model point~$m_j$.
	To address this, we marginalize the likelihood over the entire model~$m$.
	The likelihood of one data point~$d_i$ being observed from the model~$m$ can
	be expressed as the sum of the likelihood contributed from all model points
	$m_j$:
	\begin{equation}
	L(d_i | m) = \sum_j L(d_i | m_j).
	\end{equation}
	There are two important factors which influence~$L(d_i | m_j)$:
	\begin{itemize}

		\item[\textbf{1.}] The star formation rate. If the SFR is high at
		time~$j$, then it is proportionally more likely that the data
		point~$d_i$ is associated with the model points~$m_j$ than at another
		point in time when the SFR is lower.

		\item[\textbf{2.}] The selection function of the survey. If the
		survey design is such that different regions of chemical space are
		sampled more deeply than others, this will be reflected in the
		distribution in chemical space. This can happen by proxy of selection
		in, e.g., stellar age, luminosity, or color.
	...
	effects:
	\begin{equation}
	w_j \propto C(|m_j) \dot{M}_{\star,j}
	\end{equation}
	where~$C(|m_j)$ is the completeness of the survey at a given point
	in data space~$m_j$ (in this case,~\feh,~\afe, and~\logage~if that
	information is available) and~$\dot{M}_{\star,j}$ is the SFR in the model
	at time~$j$.

	\item With the proper weighting, the likelihood of observing a data point
	$d_i$ associated with a model point~$m_j$ can otherwise be expressed as a
	classical~$e^{-\chi^2/2}$ expression:
	\begin{equation}
	\chi^2 = \Delta_{ij} C_i^{-1} \Delta_{ij}^T
	\end{equation}
	where~$\Delta_{ij} = d_i - m_j$ is the vector difference associated with
	$d_i$ and $m_j$ in data space and~$C_i^{-1}$ is the inverse covariance
	matrix associated with the data point~$d_i$.

	\item This results in the following expression for our likelihood function:
	\begin{equation}
	\label{eq:likelihood_func}
	\ln L(d | m) = \sum_i \ln \left( \sum_j w_j \exp\left(\frac{-1}{2}
	\Delta_{ij} C_i^{-1} \Delta_{ij}^T\right) \right)
	\end{equation}

	\item Although there are variety of ways in which one could construct the
	likelihood function~$L(d | m)$, in the present paper we use the Markov
	...
	We then compute the likelihood function~$L(d | m)$ according to
	equation~\refp{eq:likelihood_func} and let~\mc~take care of the rest.
